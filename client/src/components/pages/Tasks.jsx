...
function Tasks() {
  ...
  const [sortModel,setSortModel]=useState(false)
    ...
    const sortFn =(a,b)=>{
      if(sortMethod !== ""){
        return a[sortMethod].localeCompare(b[sortMethod], 'en', { sensitivity: 'base' })
      }
      return 0
     }
  const getTasks=(data)=>{
        ...
        <div className="task__mainBox__controls flex">
          ...
          <div className="view--settings primary--dark--text">
            <ul>
              ...
              <li >
                {sortModel && <SortBox setSortMethod={setSortMethod} methods={['title','tag']}/>}
        ...
        <div className="task__mainBox__Board flex">
            <div className="boardBox  ">
              <div className="boardBox__header flex">
                <h4>To do <span>({filterData.sort((a,b)=>sortFn(a,b)).filter(task => task.state === "To do").length})</span></h4>
              ...
              <div className="boardBox__box flex">
                 {filterData?.map((task,index)=>{
                  if (task.state === 'To do'){
                    ...
                    return(
                      <TaskCard key={task._id} progressColor={progressColor} tagColor={tagColor} tag={tag} date={'30 Aug 2022'} progress={progress} tasks={tasks} {...info} taskId={task._id} number={index + 1} setDraggableEl={setDraggableEl} dropEvent={dropEvent} setDropType={setDropType}/>
                    )
                  }
                  return null
                })}
              ...
              <div className="dragArea" onDragOver={()=>{
                setDropType('To do')
              }}>
              drag your task here
              </div>
            ...
            <div className="boardBox  ">
              <div className="boardBox__header flex">
                <h4>In progresss <span>({filterData.sort((a,b)=>sortFn(a,b)).filter(task => task.state === "in progress").length})</span></h4>
              ...
              <div className="boardBox__box flex">
                {filterData?.map((task,index)=>{
                  if (task.state === 'in progress'){
                    const {tag,progress,progressColor,tagColor,tasks,...info} = task
            ...
            <div className="boardBox  ">
              <div className="boardBox__header flex">
                <h4>Done <span>({filterData.sort((a,b)=>sortFn(a,b)).filter(task => task.state === "done").length})</span></h4>
